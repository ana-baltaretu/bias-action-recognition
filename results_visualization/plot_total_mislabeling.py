...
# --- New: Count changes TO each skin color (aggregated from others) ---
incoming_differences = {sc: 0 for sc in skin_colors}
incoming_broken = {sc: 0 for sc in skin_colors}

for vid in pred_pivot.index:
    for target in skin_colors:
        target_pred = pred_pivot.loc[vid, target]
        target_result = corr_pivot.loc[vid, target]

        if pd.isna(target_pred) or pd.isna(target_result):
            continue

        difference_found = False
        mistake_found = False

        for other in skin_colors:
            if other == target:
                continue
            other_pred = pred_pivot.loc[vid, other]
            other_result = corr_pivot.loc[vid, other]

            if pd.isna(other_pred) or pd.isna(other_result):
                continue

            if other_pred != target_pred:
                difference_found = True
                if other_result == "correct" and target_result == "incorrect":
                    mistake_found = True
                break  # we only log one difference per video

        if difference_found:
            incoming_differences[target] += 1
            if mistake_found:
                incoming_broken[target] += 1

# --- Plot incoming impact per skin color ---
plt.figure(figsize=(16, 6))
x = range(len(skin_colors))
diffs = [incoming_differences[sc] for sc in skin_colors]
broken = [incoming_broken[sc] for sc in skin_colors]
safe = [d - b for d, b in zip(diffs, broken)]

plt.bar(x, safe, label="Change only raw prediction", color="goldenrod")
plt.bar(x, broken, bottom=safe, label="Correct → Wrong", color="crimson")
plt.xticks(x, skin_colors, rotation=45)
plt.ylabel("Number of Videos Changed To This Skin Color")
plt.title("Effect of Changing TO Each Skin Color (Aggregated From Others)")
plt.ylim(0, GRAPH_HEIGHT + 2)

# Annotate totals
for i, (s, b) in enumerate(zip(safe, broken)):
    total = s + b
    plt.text(i, total + 0.5, str(broken[i]), ha='center', va='bottom', fontsize=9)

plt.legend(loc='best', bbox_to_anchor=(0.5, 0., 0.5, 0.2))
plt.tight_layout()
plt.savefig(f"skin_color_incoming_change_impact_{model_name}.png")
plt.show()
