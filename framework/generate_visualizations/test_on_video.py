...
if __name__ == "__main__":
    ...
    for frame in tqdm.tqdm(extract_frames(opt.video_path), desc="Processing frames"):
        ...
        font_size = 64  # Set the desired font size
        try:
            font = ImageFont.truetype("arial.ttf", font_size)  # Specify the font path
        except IOError:
            font = ImageFont.load_default()  # Use the default font if the specified one is not available

        # Draw label on frame
        d = ImageDraw.Draw(frame)
        d.text(xy=(10, 10), text=predicted_label, fill=(255, 255, 255), font=font)

        output_frames += [frame]

    # Create video from frames
    writer = skvideo.io.FFmpegWriter("output/" + opt.output_file + ".gif")
